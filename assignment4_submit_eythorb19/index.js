...
function myAuthorizer(username, password) {
    if (username === 'admin' && sha256(password) === '2bb80d537b1da3e38bd30361aa855686bde0eacd7162fef6a25fe97bf527a25b') {
        return true;
    } 
    return false;
}
...
app.delete(apiPath + version + '/events/:eventId', basicAuth({authorizer: myAuthorizer}), (req, res) => {
    if (!utility.isValidObjectID(req.params.eventId)) {
        return res.status(404).json({ "error": "Event not found!" });
    }
...
app.delete(apiPath + version + '/events/:eventId/bookings/:bookingId',basicAuth({authorizer: myAuthorizer}), (req, res) => {
    if (!utility.isValidObjectID(req.params.eventId)) {
        return res.status(404).json({"message": "Event not found!"});
    }
